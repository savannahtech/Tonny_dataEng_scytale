...
def get_all_pull_requests(repo_full_name: str):
...
def main(organization: str):
    ...
    repositories = get_all_repositories(organization)

    if not os.path.exists(organization):
        os.makedirs(organization)
        save_data_to_json(repositories, f"{organization}/repositories/data.json")
